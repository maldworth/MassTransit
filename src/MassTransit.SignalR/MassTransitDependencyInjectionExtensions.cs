namespace MassTransit.SignalR
{
    ...
    {
        public static void AddMassTransitBackplane(this IServiceCollection serviceCollection, params Assembly[] hubAssemblies)
        {
            var configurator = new ServiceCollectionConfigurator(serviceCollection);

            var addScopeMethod = configurator.GetType().GetMethod("AddScope");
            ...
            foreach (var hub in hubs)
            {
                foreach (var consumer in consumers)
                {
                    ...
                    var genericMethod = addScopeMethod.MakeGenericMethod(consumerType);

                    genericMethod.Invoke(configurator, null);
                }
            }
