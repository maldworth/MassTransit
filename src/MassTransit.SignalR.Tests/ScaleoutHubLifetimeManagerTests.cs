    ...
    public class ScaleoutHubLifetimeManagerTests : DoubleScaleoutBackplaneTestFixture<MyHub>
    {
        ...
        public async Task InvokeAllAsyncWithMultipleServersWritesToAllConnectionsOutput()
        {
            ...
            using (var client2 = new TestClient())
            {
                ...
                Assert.IsTrue(Backplane1Harness.Consumed.Select<All<MyHub>>().Any());
                Assert.IsTrue(Backplane2Harness.Consumed.Select<All<MyHub>>().Any());
        ...
        public async Task InvokeConnectionAsyncOnServerWithoutConnectionWritesOutputToConnection()
        {
            using (var client = new TestClient())
            {
                ...
                Assert.IsTrue(Backplane1Harness.Consumed.Select<Connection<MyHub>>().Any());
        ...
        public async Task InvokeGroupAsyncOnServerWithoutConnectionWritesOutputToGroupConnection()
        {
            using (var client = new TestClient())
            {
                ...
                Assert.IsTrue(Backplane1Harness.Consumed.Select<Group<MyHub>>().Any());
        ...
        public async Task RemoveGroupFromConnectionOnDifferentServerNotInGroupDoesNothing()
        {
            using (var client = new TestClient())
            {
                ...
                Assert.IsTrue(Backplane1Harness.Consumed.Select<GroupManagement<MyHub>>().Any());
        ...
        public async Task RemoveGroupAsyncForConnectionOnDifferentServerWorks()
        {
            using (var client = new TestClient())
            {
                ...
                var firstMessage = Backplane1Harness.Consumed.Select<Group<MyHub>>().FirstOrDefault();
                ...
                var secondMessage = Backplane1Harness.Consumed.Select<Group<MyHub>>().Skip(1).FirstOrDefault();
        ...
        public async Task InvokeConnectionAsyncForLocalConnectionDoesNotPublishToBackplane()
        {
            using (var client = new TestClient())
            {
                ...
                Assert.IsFalse(Backplane1Harness.Consumed.Select<Connection<MyHub>>().Any());
                Assert.IsFalse(Backplane2Harness.Consumed.Select<Connection<MyHub>>().Any());
        ...
        public async Task WritingToRemoteConnectionThatFailsDoesNotThrow()
        {
            using (var client = new TestClient())
            {
                ...
                Assert.IsTrue(Backplane2Harness.Consumed.Select<Connection<MyHub>>().Any());
            }
        }
